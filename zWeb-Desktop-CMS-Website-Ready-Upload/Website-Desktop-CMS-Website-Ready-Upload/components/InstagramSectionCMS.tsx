...
interface InstagramSectionCMSProps {
  homeData: HomeMainPage | null;
}

const InstagramSectionCMS: React.FC<InstagramSectionCMSProps> = ({
  homeData,
}) => {
  // Get data from CMS or use fallback
  const sectionTitle =
    homeData?.instagramSection?.sectionTitle || "Sarah on Instagram";

  // Instagram posts from CMS or fallback
  const instagramPosts = homeData?.instagramSection?.posts?.sort(
    (a, b) => a.order - b.order
  ) || [
    {
  ...
  // Function to get image URL (CMS or fallback)
  const getImageUrl = (post: any) => {
    if (post.image && post.image.asset) {
      return getSanityImageUrl(post.image).width(220).height(336).url();
    }
    return post.image || "/images/instagram/reel-1.jpg";
  };
      ...
      <div className="instagram-header">
        <h2 className="instagram-title">
          {sectionTitle.includes("Instagram") ? (
            <>
              {sectionTitle.split("Instagram")[0]}
...
export default InstagramSectionCMS;
